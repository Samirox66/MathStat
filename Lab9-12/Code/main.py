...
def plotData(data1):
    ...
    for i in range(len(data1) - 1):
        plt.plot([i, i + 1], [data1[i][0], data1[i + 1][0]], color="blue")
        # count = count + 1
        # prev_count = count
        # cur_d = t[0]
    # plt.plot([prev_count, count], [cur_d, cur_d], 'b', linewidth=2)
    plt.title('Experiment_data(Chanel_2_700nm_0.04mm.csv)')
    plt.xlabel('n')
    plt.ylabel('mV')
...
def plotIntervals(data1):
    ...
    for i in range(len(data1)):
        if (data1[i][0] < 0.037467) and (data1[i][1] > 0.037467999999999994):
...
def plotNewIntervals(data1):
    data_n = [t for t in range(1, len(data1) + 1)]
    data1 = [[data1[i][0] - 0.001178000000000002, data1[i][0] + 0.001178000000000002] for i in range(len(data1))]

    for i in range(len(data1)):
        plt.vlines(data_n[i], data1[i][0], data1[i][1], colors = "mediumspringgreen", lw = 1)
    plt.hlines(0.037554000000000004, 0, 200, colors="hotpink", lw=2)
    plt.title('Data intervals')
    plt.xlabel('n')
    plt.ylabel('mV')
...
def plotMu(data1):
    ...
    plt.axis([0.036309499999999995, 0.03877900000000001, 0, 47])
    for i in range(len(z)):
    ...
    # print(min(mV))
    # print(max(mV))
    plt.hlines(findMax(mu), 0.036309499999999995, 0.03877900000000001, 'hotpink', '--')
...
if __name__ == '__main__':
    data1 = load_csv('Chanel_2_700nm_0.04.csv')
    plotData(data1)
    plotIntervals(data1)
    plotMu(data1)
